...
class AdminMain(QWidget):
    ...
    def handleUpdateExcel(self, value):
        try:
            ...
            if self.chp != '선택' :
                if '분류표(강)' in self.df.columns :
                    print('chp ok')
            if self.sub != '선택' :
                if '분류표(학습내용)' in self.df.columns :
                    print('sub ok')
            if self.dif != '선택' :
                if '난이도' in self.df.columns :
                    print('dif ok')
                df_mask = self.df['분류표(강)'] == 1
                self.filter = self.df[df_mask]
                print(self.filter)
            # self.currentData = self.df['분류표(강)'] == self.chp
            # df_mask = self.df['분류표(강)'] == self.chp & self.df['분류표(학습내용)'] == self.sub & self.df['난이도'] == self.dif\
            print('aaaaaaaa')
            # con_1 = self.df['분류표(강)'] == self.chp
            # con_2 = self.df['분류표(학습내용)'] == self.sub
            # con_3 = self.df['분류표(난이도)'] == self.dif
            # self.filter = self.df[con_1 & con_2 & con_3]
            # print(self.filter)
        except Exception as e :
            print(e)
    # 선택값에 따른 pd값 업데이트 끝 ------------------------------------------------------------------------------------------
